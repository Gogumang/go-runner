...
/// Answer of POST /api/device/enrollments: already usable, or waiting for approval on the admin 기기 page.
public enum DeviceEnrollmentStatus: Equatable, Sendable {
    case registered
    case pending
}
...
public enum DeviceTrustResponseParser {
    ...
    public static func parseEnrollment(status: Int, body: Data) throws -> DeviceEnrollmentStatus {
        guard (200..<300).contains(status) else { throw collectorError(status: status, body: body) }
        ...
        default: throw DeviceTrustError.malformedResponse
        }
    }
