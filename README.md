...
- **어드민**(메뉴 맨 위): 이 Mac의 Secure Enclave 키로 collector에 기기를 증명하고 어드민을 브라우저로 엽니다.
  연결 상태를 점 색으로 보여 줍니다 (회색 연결 안 됨 · 초록 연결됨 · 빨강 신호 실패). 연결된 동안 1분마다 신호를 보내고,
  **연결 끊기**를 누르거나 앱을 끄면 3분 안에 어드민이 잠깁니다.
  처음 쓰는 Mac은 설정 → 일반 → 기기 신뢰에서 **이 Mac 등록 요청**을 누르고, 이미 등록된 Mac의 어드민 관리 → 기기에서 승인합니다.
